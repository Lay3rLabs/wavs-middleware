...
forge script eigenlayer/script/WavsMirrorListOperators.s.sol -vvv --broadcast || handle_error "Failed to list operators"

echo "Operator list:"
cat "deployments/wavs-mirror/list-operators-$MIRROR_CHAIN_ID.json" | jq .

# Save operator list data
save_deployment_data "$HOME/.nodes/mirror-list-operators-$MIRROR_CHAIN_ID.json" "$(cat "deployments/wavs-mirror/list-operators-$MIRROR_CHAIN_ID.json")"
