...
DEFAULT_REGISTRY_ADDRESS=$(jq -r '.addresses.avsRegistrar' "$HOME/.nodes/avs_deploy.json" 2>/dev/null || true)
...
forge script script/eigenlayer/ecdsa/UnpauseWavsRegistration.s.sol --rpc-url "$RPC_URL" --private-key "$FUNDED_KEY" --broadcast --skip-simulation || handle_error "Failed to unpause WAVS registration"
